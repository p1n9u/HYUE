...
void *left_handed_philosopher(void *_phil) {
  int phil = *((int *)_phil);
  ...
  do {
    ...
    sem_post(&forks[phil]);
    print_fork(phil, phil, "put down");
    sem_post(&forks[(phil + 1) % N]);
    print_fork(phil, (phil + 1) % N, "put down");
...
void *right_handed_philosopher(void *_phil) {
  int phil = *((int *)_phil);
...
int main() {
  ...
  /* Only one philosopher can use print_state at a time. */
  ...
  name[0] = 0;
  pthread_create(&thread_id[0], NULL, left_handed_philosopher, &name[0]);
  for (int i = 1; i < N; i++) {
    name[i] = i;
    pthread_create(&thread_id[i], NULL, right_handed_philosopher, &name[i]);
  }
